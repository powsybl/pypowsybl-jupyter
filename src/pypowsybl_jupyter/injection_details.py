...
def build_injection_data(
    ...
    vls_df = network.get_voltage_levels()
    ...
    for vl_id in vls_df.index:
        ...
        for bus_id in vl_buses.index:
            bus_data = get_bus_injection_details(
                bus_id, injections_uncertainties, injections_filtered_df, generators_df, secondary_control
            )
            for gen in bus_data["generators"]:
                ...
                vl_gens.append({
                    "id": gen["id"],
                    "p": float(gen["p"]),
                    "u_min": float(uncertainty[0]) if has_interval else None,
                    "u_max": float(uncertainty[1]) if has_interval else None,
                    "pmin": gen["pmin_pmax"][0],
                    "pmax": gen["pmin_pmax"][1],
                    "sfcc_pct": gen.get("sfcc_pct"),
                })
        ...
        if vl_gens or vl_loads:
            result[vl_id] = {"generators": vl_gens, "loads": vl_loads}

    return result


def build_delta_data(n1: pp.network.Network, n2: pp.network.Network, epsilon: float = 1e-6) -> dict:
        ...
        for bus_id in vl_buses.index:
            ...
            for gen_id, row in bus_gens.iterrows():
                ...
                generators.append({
                    ...
                    "p_n1": float(p_n1),
                    "p_n2": float(p_n2),
                    "delta": float(p_n2 - p_n1),
                    "pmin": float(gen_row["min_p"]),
                    "pmax": float(gen_row["max_p"]),
                })

        if generators:
            result[vl_id] = {"generators": generators}
