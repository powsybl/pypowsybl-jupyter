...
def network_explorer(network: Network, vl_id : str = None, use_name:bool = True, depth: int = 1,
    ...
    Args:
        ...
        on_hover_func: a callback function that is invoked when hovering on equipments in the NAD, SLD and the network-map tabs. The function parameters are the equipment id and type; It must return an HTML string. Please read what are the equipment types supported by the different diagram widget (the NAD, the SLD and the network-map), in their detailed documentation. None, the default, will display all the attributes available in the edquipment's dataframe.
